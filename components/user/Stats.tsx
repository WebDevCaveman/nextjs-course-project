  ...
  return (
    <section className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="border-accent-solid bg-background shadow-card flex flex-col items-center justify-center gap-1.5 rounded-xl border p-4">
          <HugeIcon name="interface/diamond" size={20} className="text-accent-solid" />
          <span className="text-2xl font-semibold tabular-nums">{compact.format(reputationPoints)}</span>
          <span className="text-fg-subtle text-sm">Reputation</span>
        </div>

        <div className="border-line bg-background shadow-card grid flex-1 basis-[210px] grid-cols-2 gap-3 rounded-xl border p-4">
          {[
        ...
        {BADGE_TIERS.map(({ key, label, tint }) => (
          <div
            key={key}
            className="border-line bg-background shadow-card flex flex-1 basis-[210px] items-center gap-3 rounded-xl border p-4"
          >
