...
| **Any list of tags**<br>`ul` of tag names, ± counts             | Local `TagList`: `<TagList tags={…} />` stacked, `<TagList tags={…} inline />` wrapped.<br>ul `flex pl-0 list-none` + `flex-col gap-3.5` or `flex-row flex-wrap gap-2`<br>row `flex items-center gap-3`<br>chip Tag `md` with the devicon inside, before the label<br>count `ml-auto text-sm tabular-nums text-fg-subtle`, only when the tag carries one | "Popular Tags", profile "Top Tags". **Not** the uppercase card tag row — that is the `Tag list` row above, `Tag.sm`. `flex-row` is required inline: the base layer sets `flex-direction: column` on every `ul`. A black devicon (nextjs, github) + `colored` vanishes in dark — drop `colored` there. |
